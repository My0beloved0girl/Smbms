layui.use(['form','layer','layedit','laydate','upload'],function(){
    ...
    $("#menuAjax").click(function(){
	    ...
	    var mfid = $("#mfid").val();  //请求路径
	    var murl = $("#murl").val(); //请求路径
        ...
        //ajax提交
        $.ajax({
        	url:"/LayuiExercise/Privilege?action=updatefunction",
        	type:"post",
        	data:data2,
    ...
    form.on('select(type2)', function(data){
        ...
        if(type == 3){
        	...
        	$.get("/LayuiExercise/Privilege?action=seleAllBtn",function (res) {
        		var result = eval('(' + res + ')');
        		var btntype = $('#btnType');
        ...
        $.get("/LayuiExercise/Privilege?action=Superior&type="+type,function (res) {
        	var cs = eval('(' + res + ')');
            var data = cs;
            var dom = $("#father");
            if(type == 1){
